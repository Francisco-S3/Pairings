...
class DBconnect {
  ...
  Future<bool> updateAccount(Map<String, String> map, String id) async {
    ...
    print(
        'updateAccount status:=${response.statusCode}, reason:=${response.reasonPhrase}');
    return false;
  }
  ...
  Future<List<Favorite>?> getFav(var id) async {
    var httpsUri = Uri(
        scheme: 'http',
        host: 'pairingsdbapi-env-1.eba-jcussjem.us-east-1.elasticbeanstalk.com',
        path: '/api/favorites/$id');
    ...
    if (response.statusCode == 200) {
      ...
      print(
          'getFav status:=${response.statusCode}, reason:=${response.reasonPhrase}');
      return null;
    }
  }

  Future<bool> createFav(Map<String, String> map) async {
    var headers = {'Content-Type': 'application/x-www-form-urlencoded'};
    var request = http.Request(
        'POST',
        Uri.parse(
    ...
    print(
        'createFav status:=${response.statusCode}, reason:=${response.reasonPhrase}');
    return false;
  }
}
